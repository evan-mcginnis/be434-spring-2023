...
class Synthetic:
    ...
    def __init__(self,
        ...
        self._pool = create_pool(arguments.pctgc,
                                 arguments.maxlen,
                                 arguments.seqtype)
    ...
    def write(self) -> bool:
        ...
        :return:
        """
        with open(self._outfile, "w", encoding='UTF-8') as sequences:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--seqtype',
                        ...
                        type=str,
                        help="dna or rna")
    parser.add_argument('-n', '--numseqs', required=False, default=10,
                        type=int, help="Number of sequences to create")
    parser.add_argument('-m', '--minlen', required=False, default=50, type=int,
                        help="Minimum length")
    parser.add_argument('-x', '--maxlen', required=False, default=75, type=int,
                        help="Maximum length")

    # This is a bit more flexible way of doing this that the method given
    # parser.add_argument('-p', '--pctgc', required=False,
    # type=ranged_type(float, 0, 1), help="Percent GC", default=0.5)

    parser.add_argument('-p', '--pctgc', required=False, type=float,
    ...
    # This is the code that will pass the tests,
    if not 0 < arguments.pctgc < 1:
        parser.error(f'--pctgc "{arguments.pctgc}" must be between 0 and 1')

    # If the program requires the seed, the tests won't pass, so see
    # if it is set here.
    ...
    if synth.write():
        # Assign variables to avoid long lines, so the tests pass
        seqs_num = arguments.numseqs
        seqs_type = arguments.seqtype.upper()
        outname = arguments.outfile
        print(f'Done, wrote {seqs_num} {seqs_type} sequences to "{outname}".')
